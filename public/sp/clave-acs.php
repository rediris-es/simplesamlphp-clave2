...
if (isset($_POST['samlResponseLogout'])){
    ...
    Logger::info('eIDAS - SP.ACS: Accessing SAML 2.0 - eIDAS SP Assertion Consumer Service -- CALLED FOR SLO');

    (new HTTP)->submitPOSTData(Module::getModuleURL('clave/sp/bridge-logout.php'), $_POST);
...
Logger::info('eIDAS - SP.ACS: Accessing SAML 2.0 - eIDAS SP Assertion Consumer Service');
...
Logger::debug('Metadata on acs:'.print_r($metadata,true));


//Get the hosted SP metadata
$hostedSP = Tools::getString($metadata,'hostedSP', NULL);
if($hostedSP == NULL)
    throw new Error\Exception("'hosted SP' parameter not found in $sourceId Auth Source configuration.");
$spMetadata = Tools::getMetadataSet($hostedSP,"clave-sp-hosted");
Logger::debug('Clave SP hosted metadata: '.print_r($spMetadata,true));
...
if(!isset($_REQUEST['SAMLResponse']))
   	throw new Error\BadRequest('No SAMLResponse POST param received.');
...
$state = Auth\State::loadState($id, 'clave:sp:req');
Logger::debug('State on ACS:'.print_r($state,true));
...
$allowedRespPostParams = Tools::getArray($spMetadata,'sp.post.allowed', array());
...
if($certData !== NULL){
    Logger::debug("Remote IdP Certificate as stored in the metadata (legacy parameter): ".$certData);
...
$spkeypem  = Tools::readCertKeyFile(Tools::getString($spMetadata,'privatekey', NULL));
$expectEncrypted = Tools::getBoolean($spMetadata,'assertions.encrypted', true);
$onlyEncrypted   = Tools::getBoolean($spMetadata,'assertions.encrypted.only', false);

$eidas->setDecipherParams($spkeypem,$expectEncrypted,$onlyEncrypted);
//Logger::debug("Private Key loaded from hosted SP metadata: ".$spkeypem);
...
if($eidas->isSuccess($statusInfo)){
    ...
    if($SPsubdialect === "clave-1.0"){
        ...
        Logger::debug('Adding issuer as attribute usedIdP:'.$eidas->getRespIssuer());
    ...
    Stats::log('clave:sp:Response', $statsData);
    ...
    foreach ($respAssertions as $respAssertion){
        ...
        $idAttrName = Tools::getString($spMetadata,"idAttribute", NULL);
        ...
        if(isset($state['eidas:requestData']['issuer'])
            && $state['eidas:requestData']['issuer'] != NULL
...
if($statusInfo['MainStatusCode'] == SPlib::ATST_NOTAVAIL){
    ...
    $statusInfo['MainStatusCode'] = SPlib::ST_RESPONDER;
}
...
Stats::log('clave:sp:Response:error', $statsData);
...
Auth\State::throwException($state,
                                      new Error\Exception("IdP returned failed status: ".$statusInfo['StatusMessage']));
